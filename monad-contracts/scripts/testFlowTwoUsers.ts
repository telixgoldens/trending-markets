...
async function main() {
  ...
  // Load signers
  const deployer = new ethers.Wallet(process.env.BNB_PRIVATE_KEY!, provider);
  ...
  const binaryMarketAddr = process.env.BINARY_MARKET_ADDRESS!;
  const mockAddr = process.env.ERC20_ADDRESS!;
