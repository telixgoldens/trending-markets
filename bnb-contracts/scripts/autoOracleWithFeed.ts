...
async function main() {
  ...
  const priceFeedAddr = process.env.CHAINLINK_FEED!; 
  ...
  console.log("Setting dispute window to 20 seconds...");
  ...
  const currentPrice = Number(roundData.answer) / 1e8; 
  ...
  if (trueOutcome !== aiOutcome) {
    console.log(" Mismatch detected — challenging proposal...");
